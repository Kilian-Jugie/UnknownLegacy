...
namespace ul {
	...
	public:
		...
		*/
		...
		inline const string_t& getName() const noexcept {
			return m_Name;
		}

		template<typename _str_t>inline string_t file(_str_t&& f) {
			return m_Path.string().append("\\").append(std::forward<_str_t>(f));
		}
